...
Figure \@ref(fig:no2pcDiffcompareDay) and \@ref(fig:no2pcDiffcompareWeek) show the % difference between the daily  means for 2020 vs 2017-2019 (reference period). In either case we can see that NO2 levels in 2020 were generally _already_ lower than the reference period yet are _not_ consistently lower even during the lockdown period. The effects of covid lockdown are not clear cut...
...
compareYearsDiffPlotWeekly(o3dt[fixedDate >= myParams$comparePlotCut]) +
  labs(caption = paste0(myParams$lockdownCap))
```
> Beware seasonal trends and weather effects
