...
p <- p +
  #geom_smooth(se = FALSE) + # add smoothed line
  guides(colour = guide_legend(ncol=2)) +
...
Table \@ref(tab:NoxSummary) suggests that there may be a few (`r nrow(noxdt[value < 0])`) negative values. These are summarised in \@ref(tab:testNegNox) while Figure \@ref(fig:testNegNox) shows the availability and levels of the pollutant data over time.

```{r testNegNox, fig.cap="Oxides of nitrogen data availability and levels over time"}
t <- head(noxdt[value <0], 10)
kableExtra::kable(t, caption = "Negative NOx values (up to first 10)") %>%
  kable_styling()
...
```{r plotNoxDaily, fig.cap="Oxides of nitrogen levels, Southampton (daily mean)"}
...
```{r plotNoxDailyRecent, fig.cap="Oxides of nitrogen levels, Southampton (daily mean, recent)"}

plotDT <- noxdt[!is.na(value) & 
                  obsDate > myParams$recentCutDate, 
                .(mean = mean(value, na.rm = TRUE)),
             keyby = .(obsDate, site)]
...
```{r plotNoxHourly, fig.cap="Oxides of nitrogen levels, Southampton (hourly, last 12 months)"}
