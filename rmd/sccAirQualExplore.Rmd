...
addLockdownDate <- function(p){
  ...
  p <- p + annotate("text", x = lubridate::date("2020-03-24"),
             y = yMax * 0.4, angle = 10,size = myTextSize,
             label = "UK covid lockdown", hjust = 0)
  p <- p + annotate("rect", xmin = lubridate::as_date("2020-03-24"),
             xmax = lubridate::as_date("2020-04-10"), 
             ymin = yMin, ymax = yMax, 
             alpha = rectAlpha, 
             fill = vLineCol, 
             colour = vLineCol)
...
addLockdownDateTime <- function(p){
  ...
  p <- p + annotate("text", x = lubridate::as_datetime("2020-03-24 00:00:00"),
             y = yMax * 0.4, angle = 10,size = myTextSize,
             label = "UK covid lockdown", hjust = 0)
  p <- p + annotate("rect", xmin = lubridate::as_datetime("2020-03-24 00:00:00"),
             xmax = lubridate::as_datetime("2020-04-10 00:00:00"), 
             ymin = yMin, ymax = yMax, 
             alpha = rectAlpha, 
             fill = vLineCol, 
             colour = vLineCol) 
...
plotDT <- no2dt[!is.na(value) & obsDate > lubridate::as_date("2020-02-01"), .(mean = mean(value, na.rm = TRUE)),
             keyby = .(obsDate, site)]
...
```{r no2FromFeb1, fig.cap="Nitrogen Dioxide levels, Southampton (hourly, since Feb 1st 2020"}
...
plotDT <- so2dt[!is.na(value) & obsDate > lubridate::as_date("2020-02-01"), .(mean = mean(value, na.rm = TRUE)),
             keyby = .(obsDate, site)]
...
```{r so2FromFeb1, fig.cap="Sulphour Dioxide levels, Southampton (hourly, since Feb 1st 2020"}
...
plotDT <- o3dt[!is.na(value) & 
                 obsDate > lubridate::as_date("2020-02-01"), .(mean = mean(value, na.rm = TRUE)),
             keyby = .(obsDate, site)]
...
```{r 03FromFeb1, fig.cap="03 levels, Southampton (hourly, since Feb 1st 2020"}
recentDT <- o3dt[!is.na(value) & obsDate > lubridate::date("2020-02-01")]
p <- makeDotPlot(recentDT, 
                 xVar = "dateTimeUTC", 
                 yVar = "value", 
                 byVar = "site", 
...
```{r pm10FromFeb1, fig.cap="PM10 levels, Southampton (hourly, since Feb 1st 2020"}
...
```{r pm25FromFeb1, fig.cap="PM2.5 levels, Southampton (hourly, since Feb 1st 2020"}
...
t <- lDT[pollutant == "no2" & value > myParams$hourlyNo2Threshold_WHO][order(-value)]
