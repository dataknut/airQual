...
```{r theilSenNO2, fig.cap="Theil-Sen de-seasoned trend (NO2)"}
...
```{r theilSenNOx, fig.cap="Theil-Sen de-seasoned trend (NOx)"}
...
```{r theilSenSO2, fig.cap="Theil-Sen de-seasoned trend (SO2)"}
so2dt[, date := as.Date(dateTimeUTC)]  # set date to date for this one

oaSO2 <- openair::TheilSen(so2dt[date < as.Date("2020-06-01")], "value", 
...
```{r theilSenO3, fig.cap="Theil-Sen de-seasoned trend (O3)"}
...
```{r theilSenPM10, fig.cap="Theil-Sen de-seasoned trend (PM10)"}
...
```{r theilSenPM25, fig.cap="Theil-Sen de-seasoned trend (PM2.5)"}
...
```{r A33autocorrelation, echo = TRUE}
...
```{r A33homoskedasticity, echo = TRUE}
...
```{r centreAutocorrelation, echo = TRUE}
message("# autocorrelation/independence of errors")
...
```{r centreHomoskedasticity, echo = TRUE}
message("# homoskedasticity: plot (should be no obvious pattern")
...
```{r centreCollinearity, echo = TRUE}
message("# -> collinearity")
car::vif(lmCentre)
# if any values > 10 -> problem
message("# -> tolerance")
