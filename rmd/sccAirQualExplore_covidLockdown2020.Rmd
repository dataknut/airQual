...
pm10dt[, date := obsDate]
pm10dt[, pm10 := value]
openair::TheilSen(pm10dt[site %like% "Centre"], "pm10", ylab = "PM10", deseason = TRUE, type = c("site", "weekday"))

```

```{r TheilSenPM10A33, fig.cap="Theil-Sen  plot by weekday for PM10 at Southampton A33 site, data points are months"}

openair::TheilSen(pm10dt[site %like% "A33"], "pm10", ylab = "NO2 (ppb)", deseason = TRUE, type = c("site", "weekday"))
...
openair's timeVariation function provides plots by time of day and weekday. Figure \@ref(fig:timeVariationPM10Centre) shows the pattern for PM10 in Southampton Centre over the Figure \@ref(fig:timeVariationPM10A33) shows the same plot but for the A33 site. In each case we compare the period before lockdown (from the start of `r cutYear`) with the period since lockdown (from `r myParams$lockDownStartDate`).

These quite clearly show the difference in pollution for pre and during lockdown for specific times of day and days of the week. Unlike NO2, PM10 has not declined presumably because it is not affected by reduced transport use etc.
...
openair::timeVariation(pm10dt[site %like% "Centre" & year >= cutYear], "pm10", ylab = "PM10", group = "lockdown")
...
openair's Theil-Sen function provides de-seasoned trendlines. Figure \@ref(fig:TheilSenPM25Centre) shows the trend for PM2.5 in Southampton Centre over the last 3 years while Figure \@ref(fig:TheilSenPM25A33) shows the same plot but for the A33 site.

```{r TheilSenPM25Centre, fig.cap="Theil-Sen  plot for PM2.5 at Southampton Centre, data points are months"}
# looking for wide data
pm25dt[, date := obsDate]
pm25dt[, pm25 := value]
openair::TheilSen(pm25dt[site %like% "Centre"], "pm25", ylab = "PM2.5", deseason = TRUE, type = c("site", "weekday"))

```

```{r TheilSenPM25A33, fig.cap="Theil-Sen  plot by weekday for PM2.5 at Southampton A33 site, data points are months"}

openair::TheilSen(pm25dt[site %like% "A33"], "pm25", ylab = "PM25", deseason = TRUE, type = c("site", "weekday"))
...
openair's timeVariation function provides plots by time of day and weekday. Figure \@ref(fig:timeVariationPM25Centre) shows the pattern for PM25 in Southampton Centre over the Figure \@ref(fig:timeVariationPM25A33) shows the same plot but for the A33 site. In each case we compare the period before lockdown (from the start of `r cutYear`) with the period since lockdown (from `r myParams$lockDownStartDate`).

These quite clearly show the difference in pollution for pre and during lockdown for specific times of day and days of the week. As with PM10, PM2.5 has not declined presumably because it is not affected by reduced transport use etc.
...
```{r timeVariationPM25A33, fig.cap=paste0("timeVariation plots for PM10 at Southampton A33 site comparing lockdown 2020 with pre-lockdown starting in ", cutYear)}
openair::timeVariation(pm10dt[site %like% "A33" & year > cutYear], "pm10", ylab = "PM10", group = "lockdown")
```
