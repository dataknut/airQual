...
```{r plotNoxHourly, fig.cap="Nitric Oxide levels, Southampton (hourly)"}
...
```{r plotNoxDaily, fig.cap="Nitric Oxide levels, Southampton (daily mean - use mouse to hover over data)"}
...
```{r plotPM10Hourly, fig.cap="PM10 levels, Southampton (hourly - use mouse to hover over data)"}
...
Table \@ref(tab:pm25Summary) suggests that there may be a few (`r nrow(dt[pm2_5 < 0])`) negative values. These are shown in \@ref(tab:testNegpm25).
...
```{r plotPM25Daily, fig.cap="PM2_5 levels, Southampton (daily mean, WHO daily threshold shown in red - use mouse to hover over data)"}
dt[, obsDate := lubridate::date(obsDateTime)]
